...
if ($result->num_rows > 0) {
    while ($row = $result->fetch_assoc()) {
        ...
        echo "<tr><td>" . $row["name"] . "</td><td>" . $row["autor"] . "</td><td>" . $row["ton"] . "</td><td><a href='$pdfPath' download class='btn btn-primary'>PDF herunterladen</a></td><td><form method='post' action=''><input type='hidden' name='id' value='" . $row["id"] . "'><input type='hidden' name='action' value='delete'><button type='submit' class='btn btn-danger'>Löschen</button></form></td></tr>";
    }
} else {
